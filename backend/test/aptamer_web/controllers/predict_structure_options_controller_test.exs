defmodule AptamerWeb.PredictStructureOptionsControllerTest do
  ...
  test "does not show resource and instead throw error when id is nonexistent", %{conn: conn} do
    assert_error_sent 404, fn ->
      get(
        conn,
        predict_structure_options_path(conn, :show, "11111111-1111-1111-1111-111111111111")
      )
    end
  end
  ...
  test "creates and renders resource when data is valid", %{conn: conn} do
    ...
    created =
      Repo.get_by(PredictStructureOptions, %{
  ...
  test "does not create resource and renders errors when data is invalid", %{conn: conn} do
    conn =
      post conn, predict_structure_options_path(conn, :create), %{
        "meta" => %{},
        "data" => %{
          "type" => "predict-structure-options",
  ...
  test "updates and renders chosen resource when data is valid", %{conn: conn} do
    ...
    conn =
      put conn, predict_structure_options_path(conn, :update, predict_structure_options), %{
        "meta" => %{},
        "data" => %{
          "type" => "predict-structure-options",
